	...
	<div class="container">
		...
		<div class="row gy-2">
			...
			<ul class="nav nav-tabs" id="myTab" role="tablist">
			  ...
			  <li class="nav-item" role="presentation">
			    <button class="nav-link" id="profile-tab" data-bs-toggle="tab" data-bs-target="#profile-tab-pane" type="button" role="tab" aria-controls="profile-tab-pane" aria-selected="false" onclick="showMarkerList()">마커 종류</button>
			  </li>
			  <li class="nav-item" role="presentation">
			...
			<div class="tab-content p-2" id="myTabContent">
			  <div class="tab-pane fade show active" id="home-tab-pane" role="tabpanel" aria-labelledby="home-tab" tabindex="0">
			    <!-- 맵 설정 편집 -->
			      <form class="p-2" onsubmit="searchPlaces(); return false;" >
		         ...
		         <div class="form-check form-switch mb-3">
				    <input class="form-check-input" type="checkbox" role="switch" id="openYn">
				    <label class="form-check-label" for="openYn">공개여부</label>
				  </div>
		         </form>
		          <button type="button" class="btn btn-outline-primary" onclick="saveConfirm()" id="saveMap">수정완료</button>
	...
	<!--                    <button onclick="addWaypoint()" class="btn btn-primary">추가</button> -->
		...
		<div class="row gy-2 collapse" id="pre-map">
			...
			<ul class="nav nav-tabs" id="pre-myTab" role="tablist">
			  ...
			  <li class="nav-item" role="presentation">
			    <button class="nav-link" id="pre-profile-tab" data-bs-toggle="tab" data-bs-target="#pre-profile-tab-pane" type="button" role="tab" aria-controls="profile-tab-pane" aria-selected="false" onclick="showMarkerList()">마커 종류</button>
			  </li>
			  <li class="nav-item" role="presentation">
...
function initPreMap(level, latitude, longitude) {
...
$('#pre-map').on('shown.bs.collapse', function () {
    ...
    if (markerInfoList.length > 0) {
    	console.log("수정전 지도 !");
    	initPreMap(level, latitude, longitude);
    	displayPreMap(markerInfoList, lineList);
    }
});
...
function displayPreMap(markers, lines) {
    ...
    for (var i = 0; i < markerInfoList.length; i++) {
        ...
        // 인포윈도우를 생성합니다.
        var infoinit = new kakao.maps.InfoWindow({
            content: iwContent,
        });

        // 인포윈도우를 표시합니다.
        infoinit.open(preMap, marker);
...
function displayMap(markers, lines) {
    ...
    for (var i = 0; i < markerInfoList.length; i++) {
		...
		marker.setMap(map);
        // 인포윈도우에 표시할 내용
        var iwContent = updateInfo(markerInfoList[i].id, markerInfoList[i].content);
        ...
        // 마커에 클릭 이벤트를 등록합니다.
        (function(marker, infoinit, index) {
            kakao.maps.event.addListener(marker, 'click', function() {
                    console.log(marker);
                    marker.setMap(null); // 마커를 지도에서 제거
                    infoinit.close(); // 인포윈도우를 지도에서 제거
                    markerInfoList = markerInfoList.filter(function(item) {
            ...
            kakao.maps.event.addListener(marker, 'mouseover', function() {
                infoinit.open(map, marker); // 마커에 mouseover 시 인포윈도우를 열기
            });
        })(marker, infoinit, markerInfoList[i].id);
    }
    ...
    markerIdCounter = markerInfoList.reduce(function(max, item) {
...
function saveMap(check) { //공개 비공개 추가
  ...
  console.log(mapIdx);
  var isChecked = $("#openYn").is(":checked");
