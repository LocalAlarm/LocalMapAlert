...
.mb-3 {
	margin-bottom: 1rem;
	/* Adjust this value to increase/decrease the spacing */
}
</style>
<script>
//중복체크
...
function authenticComp() {
    ...
    if (input == code) {
        ...
        authenticCheck = true; // 인증번호 일치
    } else {
        $("#authenticWord").text("인증번호가 일치하지 않습니다.").css("color", "red");
        authenticCheck = false; // 인증번호 불일치
    }
}
...
function nicknameOk(frm) {
    ...
    if (!nicknamePattern.test(nickname)) {
        ...
        nicknameSurvey = false; // 닉네임 형식 불일치
    } else {
        $("#nicknameWord").text("사용가능한 닉네임입니다.").css("color", "#0404B4");
        nicknameSurvey = true; // 닉네임 형식 일치
    }
}
...
function joinValidate() {
    ...
    if (email == "") {
        ...
        showDangerAlert('오류', '복구 이메일을 입력해주세요!', '');
        $("#recoverEmail").focus();
        return false;
    }
		...
		<form action="join" id="joinForm" method="post"
			...
			<div class="mb-3">
				<input type="email" class="form-control" id="email" name="email"
					title="이메일" placeholder="이메일 입력" style="margin-bottom: 10px;">
				<input type="button" class="btn btn-outline-info"
					id="emailDuplicate" value="이메일 중복 체크"
					onclick="checkEmail(this.form)" style="margin-top: auto;" /> <input
					type="hidden" class="btn btn-outline-info" id="emailReset"
					value="다시 입력" onclick="resetEmail()" disabled />
			</div>
			<div class="mb-3" style="margin-bottom: 20px !important;">
				<input type="hidden" class="btn btn-outline-info mb-3"
					id="authentic" value="본인인증" onclick="authenticEmail()"
					style="margin-top: auto;"> <input type="hidden"
					class="btn btn-outline-info mb-3" id="authenticReset"
					value="다시 인증하기" onclick="resetAuthentic()" /> <input type="hidden"
					class="form-control" id="authenticNumber" name="authenticNumber"
					title="인증하기" placeholder="인증번호 입력" disabled
					onblur="authenticComp()"> <span id="authenticWord"></span>
			</div>
			<div class="mb-3" style="margin-bottom: 20px !important;">
				<input type="password" class="form-control" id="password"
					name="password" title="비밀번호" placeholder="비밀번호 입력">
			</div>
			<div class="mb-3" style="margin-bottom: 20px !important;">
				<input type="password" class="form-control" id="passwordCheck"
					name="passwordCheck" title="비밀번호 확인" placeholder="비밀번호 확인"
					onblur="passwordOk(this.form)"> <span id="passwordWord"></span>
			</div>
			<div class="mb-3" style="margin-bottom: 20px !important;">
				<input type="text" class="form-control" id="nickname"
					name="nickname" title="닉네임" placeholder="닉네임"
					onblur="nicknameOk(this.form)"> <span id="nicknameWord"></span>
			</div>
			<div class="mb-3" style="margin-bottom: 20px !important;">
				<input type="text" class="form-control" id="sample6_address"
					name="address" placeholder="주소" style="margin-bottom: 10px;">
				<input type="button" class="btn btn-outline-warning mb-3"
					value="주소 검색" onclick="sample6_execDaumPostcode()"
					style="margin-top: auto;"><br> <input type="text"
					class="form-control" name="detailAddress"
					id="sample6_detailAddress" placeholder="상세주소">
				<!--             <input type="text" name="extraAddress" id="sample6_extraAddress" placeholder="참고항목"> -->
				<div id="map"
					style="width: 270px; height: 350px; margin-top: 10px; display: none"></div>
			</div>
			<div class="mb-3" style="margin-bottom: 20px !important;">
				<input type="text" class="form-control" id="recoverEmail"
					name="recoverEmail" title="복구이메일" placeholder="복구이메일 입력">
			</div>
			<div class="mb-3" style="margin-bottom: 20px !important;">
			...
			<div class="d-flex justify-content-center mb-2 px-3">
				<input type="submit" class="btn btn-primary me-2"
					...
					value="회원가입 하기"> <input type="button"
					class="btn btn-light ms-2" value="뒤로가기" onclick="goBack()">
			</div>
		</form>
	</div>
</body>
